  ...
  <div class="page">
    <div class="container">
      <div class="page-header">
        <div>
          ...
          <h1>
            ...
            <span v-else>Interpretaciones dirigidas por {{ directorNombre }}</span>
          </h1>
        </div>
      </div>
      ...
      <div v-if="!loading && items.length === 0" class="empty-message">
        <p>No se encontraron interpretaciones para este director.</p>
...
import { getInterpretacionesPorDirector } from '../api/interpretaciones.js'
import { getDirector } from '../api/directores.js'
...
async function loadDirector() {
  ...
  try {
    const res = await getDirector(id.value)
    const data = res.data
    directorNombre.value = `${data.nombre || ''} ${data.apellido || ''}`.trim()
  } catch {
    directorNombre.value = `Director #${id.value}`
  } finally {
...
async function loadInterpretaciones() {
  loading.value = true
  try {
    const res = await getInterpretacionesPorDirector(id.value)
    const data = Array.isArray(res.data) ? res.data : (res.data?.content ?? res.data?.data ?? [])
    items.value = data
  } catch {
    alert.type = 'error'
    alert.message = 'Error al cargar las interpretaciones del director.'
  } finally {
    loading.value = false
  }
}
